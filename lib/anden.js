...
Anden.prototype.run = function(config) {
    ...
    if(!this.loaded){
        if(this.missingModules.length) throw new Error('Anden missing required modules; '+this.missingModules.join(','));
        throw new Error('Anden unable to run!');
    }

    require('./default').call(this);
